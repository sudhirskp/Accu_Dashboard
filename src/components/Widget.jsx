...
//         <div style={{fontSize:13, color:'#333'}}>{widget.text}</div>
//       </div>
//       <div style={{display:'flex', flexDirection:'column', gap:8}}>
//         <button title="Remove" onClick={()=>dispatch(removeWidget({ categoryId, widgetId: widget.id }))} style={{border:'none', background:'transparent', cursor:'pointer'}}>✖️</button>
//       </div>
//     </div>
//   )
// }
...
// return (
// <div className="bg-white p-4 rounded-lg shadow relative h-40">
// <button className="absolute top-2 right-2 text-gray-400 hover:text-red-500" onClick={()=>dispatch(removeWidget({ categoryId, widgetId: widget.id }))}>✖</button>
// <h3 className="text-sm font-medium mb-2">{widget.name}</h3>
// {widget.type === 'chart' && widget.chartType === 'donut' && (
// <DonutChart data={widget.data} labels={widget.labels}/>
// )}
// {widget.type === 'empty' && (
// <div className="flex flex-col items-center justify-center text-gray-400 h-full">No graph data available</div>
// )}
// {widget.type === 'text' && (
// <p className="text-gray-700 text-sm">{widget.text}</p>
// )}
// </div>
// )};
